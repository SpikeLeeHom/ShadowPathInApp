...
int bind_port(const char *hostnam, int portnum, jb_socket *pfd)
{
   ...
   *pfd = fd;
    log_error(LOG_LEVEL_ERROR, "bindport : %d",fd);
   return 0;
...
#define client (csp->tcp_addr)
   ...
   {
      log_error(LOG_LEVEL_ERROR, "0 == max_selected_socket");
      return 0;
   }
   do
   {
   ...
   fd = fds[i];
    log_error(LOG_LEVEL_ERROR, "accept connection fd - %d,i = %d",fd,i);
   ...
   if (afd < 0)
   {
      log_error(LOG_LEVEL_ERROR, "afd < 0 in accept");
      return 0;
   }
